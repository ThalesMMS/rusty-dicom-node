...
Do not cut `1.0.0` until the repository is comfortable treating the CLI/TUI as a stable operator-facing tool.

The original version of this checklist referenced internal issue numbers. Replace those with concrete, verifiable gates that can be checked in CI or during release prep:

- [ ] End-to-end interoperability coverage exists for: C-FIND (query), C-MOVE (retrieve), C-STORE (send), and local ingest/indexing (with at least one real PACS/test server or a well-scoped integration harness).
- [ ] Long-running TUI operations (query/retrieve/import/send) provide responsive progress/task state and the UI remains usable.
- [ ] Logs are persisted in the configured logs directory and are sufficient for diagnosing real PACS interactions.
- [ ] Node configuration validation and defaults are documented and behave predictably across CLI and TUI.
- [ ] README, CLI help, and TUI behavior are consistent (no contradictory flags/commands or workflow steps).
