...
- ZIP import streams each entry into a staged temp file in 64 KiB chunks (no full in-memory buffering);
- DICOM parsing happens from the staged file (no ZIP-entry Vec clone before parsing);
...
- [x] ZIP imports have size/count guards
- [x] Memory copies are reduced where possible (ZIP entries are no longer buffered fully in RAM during import)
